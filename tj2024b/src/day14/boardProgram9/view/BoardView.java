...
public class BoardView {
	...
	//+입력객체(현재 클래스 내부의 모든 함수들에서 사용하기 위해 메머변수에 선언)
	...
	public void run() {
		while(true) {
			System.out.println("게시물1C 2.R 3.U 4.D :");
	...
	public void update() {
		...
		//[1]수정할 정보들을 입력받는다.
		System.out.print("update Index:"); int uIndex = scan.nextInt();
		System.out.print("update Title:"); String title = scan.next();
		System.out.print("update content:"); String content = scan.next();
		System.out.print("update writer:"); String writer = scan.next();
		//[2]입력받은 값들을 객체로만든다.
		BoardDto boardDto = new BoardDto(title , content, writer);
